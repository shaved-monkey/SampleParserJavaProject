...
class AppleTest {
    ...
    private val retriever = Apple()
    private val minecraftUrl = "https://apps.apple.com/us/app/minecraft/id479516143"
    ...
    @Test
    fun `The correct url should be parsable by Apple`() {
        Assertions.assertTrue(retriever.compatibleWith(minecraftUrl))
    }

    @Test
    fun `An invalid url should not be parsable by Apple`() {
        Assertions.assertFalse(retriever.compatibleWith("https://play.google.com/store/apps/details?id=com.mojang.minecraftpe&hl=en-US"))
    }

    @Test
    fun `Actual call to AppleAppStore for Minecraft`() {
